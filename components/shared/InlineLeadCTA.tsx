...
export function InlineLeadCTA({ variant }: InlineLeadCTAProps) {
  ...
  return (
    <Link
      href={href}
      className="group block rounded-xl border border-white/8 bg-surface-1 px-5 py-4 transition-all duration-300 hover:border-white/15 hover:bg-surface-2"
    >
      <div className="flex items-center gap-4">
        <div className="shrink-0 w-1 self-stretch rounded-full bg-gradient-to-b from-accent to-accent/40" />
        ...
        <ArrowRight className="h-4 w-4 text-zinc-500 group-hover:text-accent-light group-hover:translate-x-1 transition-all shrink-0" />
      </div>
    </Link>
  );
}
