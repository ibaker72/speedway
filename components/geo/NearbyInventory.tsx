          ...
          <Link key={vehicle.id} href={`/inventory/${vehicle.slug}`} className="group block rounded-xl border border-white/5 bg-[#101010] hover:border-white/15 transition-colors">
            ...
            <div className="p-4 space-y-2">
              ...
              <div className="flex flex-wrap gap-2 text-[11px] text-zinc-400">
                <span className="inline-flex items-center gap-1 rounded-full bg-white/[0.04] px-2 py-1"><Gauge className="h-3 w-3" />{formatMileage(vehicle.mileage)}</span>
                <span className="inline-flex items-center gap-1 rounded-full bg-white/[0.04] px-2 py-1"><Fuel className="h-3 w-3" />{vehicle.fuelType || "Gasoline"}</span>
                <span className="inline-flex items-center gap-1 rounded-full bg-white/[0.04] px-2 py-1"><Cog className="h-3 w-3" />{vehicle.drivetrain || "FWD"}</span>
              </div>
            </div>
          </Link>
        ))}
