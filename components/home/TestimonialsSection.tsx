  ...
  return (
    <div className="flex items-center gap-0.5">
      {Array.from({ length: 5 }).map((_, i) => (
        <Star
          key={i}
          className={`h-4 w-4 ${i < rating ? "fill-accent text-accent" : "text-zinc-200"}`}
        />
      ))}
    </div>
  );
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {testimonials.slice(0, 3).map((t, i) => (
          <AnimateIn key={t.id} delay={i * 120} variant="up">
            <div className="relative rounded-2xl border border-zinc-200/80 p-7 bg-white hover:shadow-lg hover:border-zinc-300 transition-all duration-300 h-full">
              ...
              <span className="absolute top-3 left-5 text-7xl font-display text-zinc-100 leading-none select-none pointer-events-none">
                &ldquo;
              </span>

              <div className="relative">
                <StarRating rating={t.rating} />
                <blockquote className="mt-5 text-sm text-zinc-600 leading-relaxed">
                  &ldquo;{t.text}&rdquo;
                </blockquote>
