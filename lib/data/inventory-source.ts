...
/**
 * STRATEGY A: Static JSON file (for demo / pitch)
 * Load inventory from the local vehicles-full module.
 */
...
/**
 * STRATEGY B: Dealer Feed API
 * Most dealer platforms (DealerClick, DealerSocket, vAuto, etc.)
 * expose inventory via XML/JSON feeds.
 */
async function fetchFromDealerFeed(): Promise<Vehicle[]> {
  ...
  const res = await fetch(FEED_URL, {
    headers: FEED_KEY ? { Authorization: `Bearer ${FEED_KEY}` } : {},
...
/**
 * STRATEGY C: Third-party aggregator API
 */
async function fetchFromAggregator(): Promise<Vehicle[]> {
  throw new Error("Aggregator not configured");
}

function normalizeVehicles(raw: Record<string, unknown>[]): Vehicle[] {
  return raw.map((item: Record<string, unknown>) => ({
    id: String(item.id || item.stock_number || ""),
    slug: generateSlug(item),
    stockNumber: String(item.stock_number || item.stockNumber || ""),
    vin: String(item.vin || ""),
    year: Number(item.year),
    make: String(item.make || ""),
    model: String(item.model || ""),
    trim: item.trim ? String(item.trim) : undefined,
    bodyType: mapBodyType(String(item.body_type || item.bodyType || "")),
    condition: "used" as const,
    price: Number(item.price || item.internet_price || 0),
    msrp: item.msrp ? Number(item.msrp) : undefined,
    internetPrice: item.internet_price ? Number(item.internet_price) : undefined,
    mileage: Number(item.mileage || item.odometer || 0),
    exteriorColor: String(item.exterior_color || item.exteriorColor || ""),
    interiorColor: String(item.interior_color || item.interiorColor || ""),
    transmission: mapTransmission(String(item.transmission || "")),
    drivetrain: mapDrivetrain(String(item.drivetrain || item.drive_type || "")),
    engine: String(item.engine || ""),
    fuelType: mapFuelType(String(item.fuel_type || item.fuelType || "")),
    images: mapImages(item.images || item.photos || []),
    ...
    isCommercial: Boolean(item.is_commercial || item.isCommercial),
    isFeatured: Boolean(item.is_featured || item.isFeatured),
    isNewArrival: Boolean(item.is_new_arrival),
    isSold: Boolean(item.is_sold || item.sold),
    dateAdded: String(
      item.date_added || item.created_at || new Date().toISOString()
    ),
  }));
}
...
function mapBodyType(raw: string): Vehicle["bodyType"] {
  ...
  if (lower.includes("sedan") || lower.includes("car")) return "sedan";
  if (
    lower.includes("suv") ||
  ...
  return raw
    .map((img: unknown, i: number) => {
      if (typeof img === "string") {
        return { url: img, alt: "", isPrimary: i === 0 };
      }
      const obj = img as Record<string, unknown>;
      return {
        url: String(obj.url || obj.src || obj.image_url || ""),
  ...
  const priceRanges = [
    {
      ...
      min: 0,
      max: 10000,
      count: vehicles.filter((v) => v.price < 10000).length,
    },
    {
      label: "$10K–$15K",
      min: 10000,
      max: 15000,
      count: vehicles.filter((v) => v.price >= 10000 && v.price < 15000).length,
    },
    {
      label: "$15K–$20K",
      min: 15000,
      max: 20000,
      count: vehicles.filter((v) => v.price >= 15000 && v.price < 20000).length,
    },
    {
      label: "$20K–$30K",
      min: 20000,
      max: 30000,
      count: vehicles.filter((v) => v.price >= 20000 && v.price < 30000).length,
    },
    {
      label: "$30K+",
      min: 30000,
      max: 999999,
      count: vehicles.filter((v) => v.price >= 30000).length,
    },
  ];
...
/**
 * Main: get inventory with filtering and pagination.
 */
export async function getInventory(
  ...
  switch (source) {
    case "feed":
      vehicles = await fetchFromDealerFeed();
      break;
    case "aggregator":
      vehicles = await fetchFromAggregator();
      break;
    default:
      vehicles = await fetchFromLocalJson();
  }
  ...
  if (filters.isCommercial !== undefined) {
    vehicles = vehicles.filter((v) => v.isCommercial === filters.isCommercial);
  }
  if (filters.search) {
  ...
  if (filters.drivetrain)
    vehicles = vehicles.filter((v) => v.drivetrain === filters.drivetrain);
